...
import 'package:my_locator/pages/model/from_locator_post.dart';
...
import 'package:my_locator/pages/menu/choice.dart';
import 'package:my_locator/pages/menu/from_locator_empty.dart';

class FromLocatorPage extends StatefulWidget {
  @override
  _FromLocatorPageState createState() => _FromLocatorPageState();
}

class _FromLocatorPageState extends State<FromLocatorPage> {
  String resultCode;
      ...
      if (response.statusCode == 200) {
        ...
        getUom(resultCode).whenComplete(() {
          ...
          Navigator.pushReplacement(
              context,
              MaterialPageRoute(
                  builder: (BuildContext context) => FromLocatorIsEmptyPage()));
        }
      } else {
                        ...
                        Container(
                          ...
                          child: ElevatedButton(
                            ...
                            onPressed: isButtonDisabled
                                ? null
                                : () async {
                                    /* var doubleQty = double.parse(resultQuantity.text);
                    assert(doubleQty is double);

                    var valOnHandStockTemp = valOnHandStock.toString();
                    var doubleOnHandStock = double.parse(valOnHandStockTemp);
                    assert(doubleOnHandStock is double); */
                                    ...
                                    if (valKodeItem == null ||
                                      ...
                                      /* var resultQuantityTemp = resultQuantity.text;
                      var doubleQty;
                      if (resultQuantityTemp.contains('.')) {
                        doubleQty = double.parse(resultQuantity.text);
                        assert(doubleQty is double);
                        print(doubleQty);
                      } else {
                        var intQty = int.parse(resultQuantity.text);
                        assert(intQty is int);
                        print(intQty);
                      }

                      var valOnHandStockTemp = valOnHandStock.toString();
                      if (valOnHandStock.toString().contains('.')) {
                        var doubleOnHandStock =
                            double.parse(valOnHandStockTemp);
                        assert(doubleOnHandStock is double);
                        print(doubleOnHandStock);
                      } else {
                        var intOnHandStock = int.parse(valOnHandStockTemp);
                        assert(intOnHandStock is int);
                        print(intOnHandStock);
                      }

                      var resultInvItemIdTemp = resultInvItemId.toString();
                      var intInvItemId = int.parse(resultInvItemIdTemp);
                      assert(intInvItemId is int);
                      print(intInvItemId);

                      var valOrgIdTemp = valOrgId.toString();
                      var intOrgId = int.parse(valOrgIdTemp);
                      assert(intOrgId is int);
                      print(intOrgId);

                      var resultPersonIdTemp = resultPersonId;
                      var intPersonId = int.parse(resultPersonIdTemp);
                      assert(intPersonId is int);
                      print(resultPersonId);

                      var valLocationIdTemp = valLocationId.toString();
                      var intLocationId = int.parse(valLocationIdTemp);
                      assert(intLocationId is int);
                      print(intLocationId);

                      print(resultLotNumber +
                          ' + ' +
                          resultId +
                          ' + ' +
                          resultCode +
                                      ...
                                      if (ConnectivityResult.none != null) {
                                        FromLocatorPost.connectToApi(
                                          ...
                                          LogPost.connectToApi(
                                              'Save Draft',
                                              'From Locator: $resultCode, Description: $resultDesc, Lot Number: $resultLotNumber, Sub Inv: $valSubInv, Org Id: $valOrgId, UOM: $valUom, Transfer Qty: ' +
                                                  resultQuantity.text,
                                              resultPersonId);

                                          setState(() {
                                            // isLoading = false;
                                            resultCode = 'Hasil QR Code';
                                            valKodeItem = null;
                                            resultLotNumber = null;
                                            resultId = null;
                                            valSubInv = null;
  ...
  Future _openScanner(BuildContext context) async {
    ...
    setState(() {
      ...
      if (resultCode != null) {
        ...
        // isLoadingUom = true;
        isLoadingUomOnHand = true;
        getKodeItem(resultCode, resultSearch.text).whenComplete(() {
          isLoadingData = false;
        }).catchError((e) {
          print('e');
          Text('Gagal loading ...');
